...
describe("Vector", () => {
    describe('2D Vector tests', () => {
        
        it("should get, set the length of a 2D vector", () => {
            let v = new Vector(3,4);
            const beforeLength = v.getLength();
            expect(beforeLength).toBe(5);
            const newLength = 10;
            v = v.setLength(newLength);
            const afterLength = v.getLength();
            expect(afterLength).toBe(newLength);

            expect(v.equals(new Vector(6,8))).toBe(true);
        });
        it("should get 2D vector angle", () => {
    ...
    describe('3D Vector tests', () => {
        ...
        it('should subtract one 3D vector from another', () => {
            let v = new Vector(1,2,3);
            let w = new Vector(4,5,6);
            expect(VectorMath.subtract(v,w)).toEqual(new Vector(-3,-3,-3));
        })
